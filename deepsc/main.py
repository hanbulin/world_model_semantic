...
def train(epoch, args, net, mi_net=None):
    train_eur= EurDataset('train', data_root=args.data_root)
...
if __name__ == '__main__':
    ...
    deepsc = DeepSC(args.num_layers, num_vocab, num_vocab,
                        ...
                        args.dff, 0.1, channel_symbols=args.channel_symbols).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        avg_val_loss = validate(epoch, args, deepsc)
        elapsed = time.time() - train_start_time
        progress = (epoch + 1) / max(args.epochs, 1)
        eta_seconds = elapsed * (args.epochs - (epoch + 1)) / max(epoch + 1, 1)
        print(
            'Epoch: {}/{}; Validation loss: {:.5f}; Epoch time: {:.2f}s; '
            'Elapsed: {}; ETA: {}'.format(
                epoch + 1,
                args.epochs,
                avg_val_loss,
        ...
        torch.save(
            deepsc.state_dict(),
            os.path.join(args.checkpoint_path, 'checkpoint_last.pth'))
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(
                deepsc.state_dict(),
                os.path.join(args.checkpoint_path, 'checkpoint_best.pth'))
            torch.save(
                deepsc.state_dict(),
                os.path.join(args.checkpoint_path, 'checkpoint_{}.pth'.format(str(epoch + 1).zfill(2))))
